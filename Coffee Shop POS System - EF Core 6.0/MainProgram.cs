...
This current state (beta) does not have any UI element nor does it have any AI elements. All inputs are to be manually given by the user (employee) for another customer");
        ...
        while (userChoice != (int)UserOptions.EndDay)
        {
            ...
            switch (userChoice)
            {
                ...
                case (int)UserOptions.Add:
                    ...
                    CustomerOrder entityOrderToAdd = new CustomerOrder { ProductCatalogue = productSet.Item2, ProductProperties = productSet.Item1 };
    ...
    private static (ProductProperties, ProductCatalogue) PromptForItem(DatabaseModel database)
    {
        ...
        ProductCatalogue chosenProductCatalogue = availableProductRecords.Single(x => x.ENname == chosenProductString);
        
        // Gets the record with the product's variation
        var productVariation = database.ProductVariants.Where(x => x.ProductCatalogueId == chosenProductCatalogue.ProductCatalogueId).ToList(); // 3 records stored
        ...
        var returnProduct = new ProductCatalogue { ENname = chosenProductString, Categories = database.Categories.Single(x => x.CategoriesId == categoryNumber) };
